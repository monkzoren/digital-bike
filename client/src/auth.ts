...
if (!firebaseEnabled) {
  ...
  console.warn(
    `[db] Player accounts are DISABLED — no Firebase config in this build ` +
...
const TOKEN_KEY = seat ? `db_token:${seat}` : 'db_token';
...
export function initAuth(): Promise<void> {
          ...
          for (let attempt = 0; ; attempt++) {
            try {
              ...
              if (!transient || attempt >= 2) {
                console.error('[db] anonymous sign-in failed', err);
                degraded = true;
                notify();
  ...
  ready = Promise.race([
    ...
    new Promise<void>(resolve =>
      setTimeout(() => {
        if (!user) {
          console.warn('[db] auth did not settle in time — continuing offline');
          degraded = true;
          notify();
        }
        resolve();
...
export async function getToken(): Promise<string | undefined> {
  ...
  if (user) {
    try {
      ...
      console.error('[db] could not mint an ID token', err);
      degraded = true;
    }
  }
...
export type SignInResult =
  ...
  /** The provider was already a Digital Bike account, so we signed into
...
export async function signInWithGoogle(): Promise<SignInResult> {
  ...
  try {
    ...
    // That Google account already has a Digital Bike account of its own.
...
// ---------------------------------------------------------------------------
const EMAIL_KEY = 'db_signin_email';
