...
//     the wrapper reports it in the user agent (DigitalBikeDesktop/<tag>).
...
const LATEST_RELEASE_API =
  'https://api.github.com/repos/monkzoren/digital-bike/releases/latest';
const DESKTOP_DOWNLOAD_URL =
  'https://github.com/monkzoren/digital-bike/releases/latest/download/DigitalBike-Setup.exe';
...
async function checkBuild() {
  ...
  try {
    ...
    if (buildId && buildId !== __BUILD_ID__) {
      newBuildSeen = true;
      showBanner('A new version of Digital Bike is live', 'REFRESH', null, () => {
        location.reload();
      });
    }
  } catch {
...
const uaMatch = navigator.userAgent.match(/DigitalBikeDesktop\/(\S+)/);
