// Touch controls: a floating 8-way stick on the left, HOP / TRICK / BOOST on
// the right. Lifted from Digital Tennis and re-buttoned for the bike.
//
...
type Handlers = {
  /** A button went down or came up. `mask` is the BTN_* bit it owns. */
  button: (mask: number, down: boolean) => void;
};
...
function initStick() {
  ...
  zone.addEventListener('pointerdown', e => {
    ...
    stick.style.display = 'block'; // measurable only once displayed
...
export function initTouch(h: Handlers) {
  ...
  initStick();
  // Bit values mirror BTN_* in config.ts / the module.
  initButton($('touch-hop'), () => h.button(1, true), () => h.button(1, false));
  initButton($('touch-trick'), () => h.button(2, true), () => h.button(2, false));
  initButton($('touch-boost'), () => h.button(4, true), () => h.button(4, false));
